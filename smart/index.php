...
/* FilmDB (based on php4flicks) */
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$_POST['count'] = (isset($_GET['count'])?$_GET['count']:9); 
	...
	if($_POST['view']=='film') {
		...
		$noe = (isset($_POST['count'])?$_POST['count']:9);
	}
	...
	if(isset($_SESSION['user'])){
		...
		$nil = 'page='.$_POST['page'].'&view='.$_POST['view'].'&genres='.$_POST['genres'].'&tosearch='.$_POST['tosearch'].'&searchin='.$_POST['searchin'].'&sorter='.$_POST['sorter'].'&sortby[0]='.$_POST['sortby'][0].'&sortby[1]='.$_POST['sortby'][1].'&sortby[2]='.$_POST['sortby'][2].'&filtertitle='.$now.'&filter='.rawurlencode($_POST['filter']).'&count='.$_POST['nof'];
		$_SESSION['tmpstr'] = $nil;
	}
...
<? }?>
	...
	<div class="shelf_area"><div class="shade"></div>
<? if($_POST['view']=='poster') { ?>
<? for($i=0; $i<=$noe; $i++) {$row=mysql_fetch_array($result); $ava=$row['avail']!='1'?'notavail':'avail'; if($row['id']!='') {?>
...
<? if($rowcount==0) {echo '<b>0 - 0</b> / <b>0</b>';} else {echo '<b>'.($_POST['page']+1).'&#45;'.($_POST['page']+min($noe,$rowcount-$_POST['page'])).'</b>&nbsp;&#47;&nbsp;<b>'.$rowcount.'</b>';} ?>
	...
	<div id="conf" style="visibility:hidden;">
		...
		<div id="level_0" class="inner">
			...
			<fieldset style="margin-top: -1px;">
				...
				<ul>
					<li class="search"><input type="text" id="title" placeholder="..." value="<?= isset($_POST['tosearch'])?$_POST['tosearch']:''?>" onkeydown="submitenter(event);" /></li>
				</ul>
			</fieldset>
			<div id="go" class="button go" onclick="tfilter(document.filterform.title.value,document.filterform.searchin.value);">&#9664;</div>
		</div>	
...
<? } ?>
		...
		<input type="hidden" name="count" value="<?= isset($_POST['count'])?$_POST['count']:'9' ?>">
...
<!--
<? echo 'var s_size='.$sortsize.',nop='.$_POST['count'].';'."\n"; ?>
-->
</script>
</body>
</html>