...
<html>
	<head>
		...
		<link rel="stylesheet" media="screen" href="landscape<? echo ($bigdev==TRUE?'_big':'') ?>.css" type="text/css" />		
		<!-- link rel="stylesheet" media="only screen and (max-device-width: 1024px)" href="landscape_big.css" type="text/css" / -->		
...
<? if($bigdev==TRUE) { /* iPad */ ?>
		<div class="shelf_row white_top"><div class="row_shade trans_50"></div><div class="row_horizontal"></div>
			<div class="row_inner">
<? for($i=0; $i<8; $i++) {if($rowcount>$i&&$noe>$i) {$row=mysql_fetch_array($result); $ava=$row['avail']!='1'?'notavail':'avail'; if($row['id']!='') {?>
...
<? }}}?>
		...
		<div class="shelf_row"><div class="row_shade trans_60"></div><div class="row_horizontal"></div>
			<div class="row_inner">
<? for($i=8; $i<16; $i++) {if($rowcount>$i&&$noe>$i) {$row=mysql_fetch_array($result); $ava=$row['avail']!='1'?'notavail':'avail'; if($row['id']!='') {?>
				<div id="p_<?= $i?>" onclick="setfilm(<?= $_POST['page']+$i ?>);" class="row_entry indicator">
					<img class="poster" src="../imgget.php?for=<?= $row['fid']?>" border="0" alt="" />
					<img class="<?= $ava ?>" src="images/na.png" border="0" alt="" />
					<span class="name">&nbsp;<? if(($_POST['sorter']=='name_ASC')||($_POST['sorter']=='name_DESC')){ echo $row['name']; }else{ echo $row['local']; }?></span>
...
<? }}}?>
		...
		<div class="shelf_row"><div class="row_shade trans_70"></div><div class="row_horizontal"></div>
			<div class="row_inner">
<? for($i=16; $i<24; $i++) {if($rowcount>$i&&$noe>$i) {$row=mysql_fetch_array($result); $ava=$row['avail']!='1'?'notavail':'avail'; if($row['id']!='') {?>
				<div id="p_<?= $i?>" onclick="setfilm(<?= $_POST['page']+$i ?>);" class="row_entry indicator">
					<img class="poster" src="../imgget.php?for=<?= $row['fid']?>" border="0" alt="" />
					<img class="<?= $ava ?>" src="images/na.png" border="0" alt="" />
					<span class="name">&nbsp;<? if(($_POST['sorter']=='name_ASC')||($_POST['sorter']=='name_DESC')){ echo $row['name']; }else{ echo $row['local']; }?></span>
...
<? }}}?>
		...
		<div class="shelf_row"><div class="row_shade trans_80"></div><div class="row_horizontal"></div>
			<div class="row_inner">
<? for($i=24; $i<32; $i++) {if($rowcount>$i&&$noe>$i) {$row=mysql_fetch_array($result); $ava=$row['avail']!='1'?'notavail':'avail'; if($row['id']!='') {?>
				<div id="p_<?= $i?>" onclick="setfilm(<?= $_POST['page']+$i ?>);" class="row_entry indicator">
					<img class="poster" src="../imgget.php?for=<?= $row['fid']?>" border="0" alt="" />
					<img class="<?= $ava ?>" src="images/na.png" border="0" alt="" />
					<span class="name">&nbsp;<? if(($_POST['sorter']=='name_ASC')||($_POST['sorter']=='name_DESC')){ echo $row['name']; }else{ echo $row['local']; }?></span>
...
<? }}}?>
		...
		<div class="shelf_row black_bottom"><div class="row_shade trans_90"></div><div class="row_horizontal"></div>
			<div class="row_inner">
<? for($i=32; $i<40; $i++) {if($rowcount>$i&&$noe>$i) {$row=mysql_fetch_array($result); $ava=$row['avail']!='1'?'notavail':'avail'; if($row['id']!='') {?>
				<div id="p_<?= $i?>" onclick="setfilm(<?= $_POST['page']+$i ?>);" class="row_entry indicator">
					<img class="poster" src="../imgget.php?for=<?= $row['fid']?>" border="0" alt="" />
					<img class="<?= $ava ?>" src="images/na.png" border="0" alt="" />
					<span class="name">&nbsp;<? if(($_POST['sorter']=='name_ASC')||($_POST['sorter']=='name_DESC')){ echo $row['name']; }else{ echo $row['local']; }?></span>
...
<? } ?>
				...
				<span class="data reverse"><?= '<b>'.$row['disks'].'</b> x <b>'.$row['medium'].'</b> [<b>'.$row['type'].'</b>] (<b>'.$row['container'].'</b>)' ?></span>
				<span class="type"><?= $trans['t_video']?></span>
				<span class="data reverse"><?= '<b>'.$row['video'].'</b> &bull; <b>'.$row['width'].'</b>x<b>'.$row['height'].'</b> &bull; <b>'.$row['format'].'</b> &bull; <b>'.$row['ratio'].'</b>'?></span>
				<span class="type"><?= $trans['t_audio']?></span>
				<span class="data reverse"><?= '<b>'.$row['audio'].'</b> &bull; <b>'.$trans[$row['channel']].'</b> &bull; <b>'.$row['herz'].'</b> <small>KHz</small>' ?></span>
			</div>
<? }?>
		</div>
<? }?>
...
<? } ?>
	...
	<div id="conf" style="visibility:hidden;">
		<form name="filterform" action="landscape.php" method="post">
		<div id="level_0" class="inner">
			...
			<fieldset>
				<legend>Sort to...</legend>
				<ul>
					<li id="sorter" onclick="switchconf('level_2-1','level_0');" class="single">&darr; <?= $trans['t_since']?></li>
					<li class="check"><big>&uarr;</big> == Ascending &nbsp; &nbsp; <big>&darr;</big> == Descending</li>
				</ul>
			</fieldset>
			<fieldset>
